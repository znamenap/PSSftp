...
namespace PSSftpProvider
{
    public partial class SftpCmdletProvider
    {
        ...
        {
            Logger.LogMethodBegin(nameof(ConvertPath), "input: path={0}; filter={1}, updatedPath={2}, updatedFilter={3}",
                path, filter, updatedPath, updatedFilter);

            updatedPath = GetSftpLocalPath(path);
            var result = true;

            Logger.LogMethodEnd(nameof(ConvertPath), "result: result={0}; updatedPath={1}, updatedFilter={2}",
        ...
        {
            Logger.LogMethodBegin(nameof(GetChildNames), "input: path={0}, retrunContainers={1}", path, returnContainers);
            base.GetChildNames(path, returnContainers);
            Logger.LogMethodEnd(nameof(GetChildNames), "void");
        }
    }
}