...
namespace SFtp.PSProvider.Tests.TestFixtures
{
    public class PowerShellExecutionResult
    {
        ...
        public PSObject[] OutputObjects { get; private set; }

        public ErrorRecord[] ErrorRecords { get; private set; }

        public PSInvocationStateInfo InvocationStateInfo { get; private set; }

        public bool ShellHadErrors { get; private set; }

        public PowerShellExecutionResult(string[] outputLines, PSObject[] outputObjects, ErrorRecord[] errorRecords, PSInvocationStateInfo invocationStateInfo, bool shellHadErrors)
        {
    ...
    public class PowerShellTestFixture
    {
        ...
        public PowerShellExecutionResult Execute(IEnumerable inputs, params string[] lines)
        {
            ...
            errorObjects.ForEach(record => Console.Error.WriteLine("{0}{1}{2}{3}", record.ToString(), Environment.NewLine,
                record.ErrorDetails, Environment.NewLine));

            return new PowerShellExecutionResult(outputLines.ToArray(), outputObjects.ToArray(), errorObjects.ToArray(), Shell.InvocationStateInfo, Shell.HadErrors);
        }
